...
export async function registerBackgroundSync() {
  if ("serviceWorker" in navigator && "SyncManager" in window) {
    ...
    console.log(
      "ℹ️ Background Sync API no disponible, usando sincronización manual",
    );
    return false;
  }
}
...
export async function syncPendingOperations() {
  ...
  try {
    ...
    console.log(
      `📋 ${operations.length} operaciones pendientes de sincronización`,
    );
    ...
    console.log(
      `🏁 Sincronización completada: ${results.synced}/${results.total} exitosas`,
    );
...
async function syncCreateOperation(operation) {
  ...
  try {
    ...
    if (dbManager.isTempUUID(uuid)) {
      ...
      console.log(
        `🔄 UUID temporal ${uuid} reemplazado por ${createdItem.uuid}`,
      );
    } else {
...
async function syncUpdateOperation(operation) {
  ...
  try {
    ...
    if (dbManager.isTempUUID(uuid)) {
      console.warn(
        `⚠️ No se puede actualizar ${entity} con UUID temporal: ${uuid}`,
      );
      return {
        success: false,
    ...
    if (!response.ok) {
      ...
      if (response.status === 404) {
        console.warn(
          `⚠️ ${entity} ${uuid} no existe en el servidor (posiblemente eliminado)`,
        );
        ...
        return {
          success: true,
          warning: "Recurso no encontrado, eliminado de caché",
        };
      }
...
async function syncDeleteOperation(operation) {
  ...
  try {
    ...
    if (dbManager.isTempUUID(uuid)) {
      console.log(
        `ℹ️ Eliminando ${entity} con UUID temporal localmente: ${uuid}`,
      );
      if (entity === "product") {
        await dbManager.deleteProduct(uuid);
      } else {
...
export function initSyncService() {
  ...
  window.addEventListener("sync-pending", async () => {
    if (getIsOnline() && !syncInProgress) {
      console.log(
        "📡 Nueva operación pendiente y hay conexión, sincronizando...",
      );
